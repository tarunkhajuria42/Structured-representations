...
if( not os.path.isdir('Features_DINO2/')):
    os.mkdir('Features_DINO2/')
    ...
    for obj1 in meta_by_objects.keys():
        for obj2 in meta_by_objects[obj1].keys():
            ...
            for ind,info in enumerate(infos):
                ...
                try:
                    inputs = processor(images=image, return_tensors="pt") # inputs for the model
                except:
                    continue
                ...
                for ob in [obj1,obj2]:
                    ...
                    resized = np.array(mask_img.resize((16,16),Image.NEAREST)).flatten() # resize to size of tokens
                    ...
                    for layer in layers:
                        ...
                        random_full = np.random.choice(np.arange(0,256))+1
                ...
                if((len(all_features)+1) % 100 == 0):
                    f = open('Features_DINO2/'+task_name+'_'+obj1+'_'+obj2+'_'+str(counter)+'.pkl','wb')
