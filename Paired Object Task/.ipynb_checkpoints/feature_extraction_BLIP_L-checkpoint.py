            ...
            for ind,info in enumerate(infos):
                ...
                for ob in [obj1,obj2]:
                    ...
                    for layer in layers:
                        ...
                        #norms = np.linalg.norm(feature[layer][0][0,:],axis=1)
                        img_dict[ob]['all_features'][layer] = np.mean(feature[layer][0][0,sel_index+1].cpu().numpy(),axis=0)
                        ...
                        #img_dict[ob]['norms'][layer] = feature[layer][0][0,np.argsort(norms)[-3:]].cpu().numpy()
                if(continue_flag):
                    continue
                all_features.append(img_dict)
                if((len(all_features)+1) % 50 == 0):
                    f = open('Features_BLIP_Large/'+task_name+'_'+obj1+'_'+obj2+'_'+str(counter)+'.pkl','wb')
