...
class FeatureExtractor(torch.nn.Module):
    def __init__(self, model, layers):
        ...
        for layer_id in layers:
            self.model.image_model.encoder.layer[layer_id].register_forward_hook(self.save_outputs_hook(layer_id))
        ...
        for obj2 in meta_by_objects[obj1].keys():
            ...
            for ind,info in enumerate(infos):
                ...
                if((len(all_features)+1) % 100 == 0):
                    f = open('Features_FLAVA/'+task_name+'_'+obj1+'_'+obj2+'_'+str(counter)+'.pkl','wb')
