import requests
from transformers import VisionEncoderDecoderModel, AutoTokenizer, AutoProcessor, BlipModel
...
class BlipCaptioningModel():
    def __init__(self, pretrained_model="Salesforce/blip-image-captioning-base", output_encoder_attentions=True, output_encoder_hidden_states=True) -> None:
    ...
    def model_initialization(self, pretrained_model, output_encoder_attentions=True, output_encoder_hidden_states=True):
        model = BlipModel.from_pretrained(pretrained_model)
    ...
    def forward_pass(self, imgs:list):
        inputs = self.processor(
            text=[""], images=imgs, return_tensors="pt", padding=True
        )
